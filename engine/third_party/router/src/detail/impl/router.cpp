...
class segment_template {
 ...
 public:
  ...
  friend bool operator==(segment_template const& a, segment_template const& b) {
    if (a.is_literal_ != b.is_literal_) return false;
    if (a.is_literal_) return a.str_ == b.str_;
  ...
  friend bool operator<(segment_template const& a, segment_template const& b) {
    ...
    if (a.is_literal()) return !b.is_literal();
    return a.modifier_ < b.modifier_;
  }
};
...
bool segment_template::match(pct_string_view seg) const {
  if (is_literal_) return *seg == str_;
...
class impl {
 ...
 public:
  ...
  ~impl() {
    for (auto& r : nodes_) delete r.resource;
  }
...
node const* impl::find_optional_resource(const node* root,
  ...
  for (auto i : root->child_idx) {
    auto& c = ns[i];
    if (!c.seg.is_optional() && !c.seg.is_star()) continue;
        ...
        if (c.seg.is_literal()) {
          ...
          *ids++ = c.seg.id();
          ...
          while (it1 != end) {
            if (*it1 == "..") {
              ++ndotdot;
              if (ndotdot >= (nnondot + c.seg.is_star())) break;
            } else if (*it1 != ".") {
              ++nnondot;
            }
            ++it1;
...
router_base::any_resource const* impl::find_impl(
    segments_encoded_view path, core::string_view*& matches,
    core::string_view*& ids) const {
  // parse_path is inconsistent for empty paths
  if (path.empty()) path = segments_encoded_view("./");
